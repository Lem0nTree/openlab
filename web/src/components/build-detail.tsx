    ...
    <div className="build-detail-grid">
      <section className="build-detail-main">
        ...
        <article className="build-panel">
          ...
          {validation ? <div className={`wiring-validation is-${validation.status}`}><div><small>DETERMINISTIC CHECK</small><strong>{validation.status?.replaceAll("_", " ")}</strong></div>{validation.errors?.map((value) => <p className="error" key={value}>{value}</p>)}{validation.warnings?.map((value) => <p className="notice" key={value}>{value}</p>)}{schematic?.erc ? <p>KiCad ERC: <strong>{schematic.erc.status}</strong>{schematic.erc.reason ? ` · ${schematic.erc.reason}` : ""}</p> : null}{latestSchematicJob && latestSchematicJob.result?.status !== "blocked" && !acceptedSchematic ? <button disabled={busy} onClick={() => void acceptSchematic(latestSchematicJob)}>Accept checked wiring</button> : null}</div> : null}
      ...
      <aside className="build-detail-aside">
        ...
        <article className="build-side-card"><p className="eyebrow">BOM REQUIREMENTS</p><div className="build-item-list">{detail.requirements.map((item) => <div className={`build-item ${item.match_status === "missing" ? "missing" : ""}`} key={item.id}><span>{item.quantity}×</span><strong>{item.name}</strong><small>{item.priority}</small></div>)}</div><form className="requirement-form compact-form" onSubmit={addRequirement}><input name="name" placeholder="Part or capability" required/><input name="quantity" aria-label="Quantity" type="number" min="0.000001" step="any" defaultValue="1" required/><select name="priority" aria-label="Priority" defaultValue="required"><option value="required">Required</option><option value="recommended">Recommended</option><option value="optional">Optional</option></select><button disabled={busy}>Add</button></form></article>
