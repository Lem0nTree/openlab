...
export function Locations() {
  ...
  return <Shell title="Locations"><form className="toolbar" onSubmit={add}><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Drawer, bin, shelf…" required/><button>Add location</button></form>{error && <p className="error">{error}</p>}<div className="list">{locations.map((location) => <article key={location.id}><img className="qr" src={`/api/v1/locations/${location.id}/qr.svg`} alt={`QR label for ${location.name}`}/><div><strong>{location.name}</strong><small>Location ID: {location.public_code}</small></div></article>)}</div></Shell>;
}
